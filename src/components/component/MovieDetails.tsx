...
export const MovieDetails = () => {
  ...
  const getMovieData = async () => {
    try {
      const { data } = await axiosInstance.get(`movie/${params.id}?language=en-US`);
      setGetData(data);
    } catch (err: any) {
  ...
  console.log(getData.poster_path," getdata");
    ...
    <div className="justify-center items-center w-screen h-screen ">
      <div className="flex justify-between ">
        <div className="flex flex-col h-[68px] w-[211px]">
          ...
          <p className="w-full h-7">{getData.release_date + " " +getData.runtime}</p>
        </div>
      ...
      <div>
        <div className="w-full flex justify-between ">
          <p className="text-[24px]">{getData.title}</p>
